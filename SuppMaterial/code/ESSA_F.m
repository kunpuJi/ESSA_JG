function signal = ESSA_F(formal_error, timeseries,index_miss,L,kk )
%% Extended singular spectrum analysis (weighted)
%  Author: Kunpu Ji 
%  Date: 2021/07/18
%  Input:  formal_error -- formal errors of time series (Ns x 1)
%          timeseries -- timeseries (Ns x 1)
%          index_miss -- gap locations ((N - Ns) x 1)
%          L -- window size
%          kk -- reconstruction order
%  Output: signal-- extracted signals (N x 1)
...
% Step 4: Constructing the weight matrix  
sigma02 = length(formal_error) / sum(1./formal_error.^2);
p_d = zeros(N,1);
p_d(index_avail) = sigma02./formal_error.^2;
p_d(index_miss) = 1;
PP = diag(p_d);

% Step 5: Solving the equation and extracting the signals
value = -(A2'*PP*A2)\(A2'*PP*A1*timeseries);
